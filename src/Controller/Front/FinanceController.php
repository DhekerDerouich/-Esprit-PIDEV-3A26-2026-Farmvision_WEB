...
public function newDepense(
    ...
    if ($request->isMethod('POST')) {
        ...
        if (count($errors) === 0) {
            ...
            // Debug info
            $this->addFlash('info', "🔍 USER ID: {$userId} | TYPE: {$typeDepense} | MONTANT: {$montant} DT");
            $this->addFlash('info', "📊 MOYENNE 3 DERNIERS MOIS: " . round($moyenne, 2) . " DT");
            ...
            $this->addFlash('info', "🎯 SEUIL (150%): {$seuil} DT");

            // ✅ FIX #2 — Debug phone number early so you can catch null
            $userPhone = $user->getTelephone();
            $this->addFlash('info', "📱 TÉLÉPHONE: " . ($userPhone ?? 'NULL — aucun numéro dans le profil'));

            // Anomaly detection
            $anomaly = $anomalyService->isAnomaly($typeDepense, $montant, $userId);

            if ($anomaly['is_anomaly']) {
                $this->addFlash('warning', "⚠️ ANOMALIE DÉTECTÉE — Écart: +{$anomaly['pourcentage']}% | Moyenne: {$anomaly['moyenne']} DT | Seuil: {$anomaly['seuil']} DT");
                ...
                if (!empty($userPhone)) {
                    ...
                    if ($result['success']) {
                        $this->addFlash('success', '📱 Alerte SMS envoyée ! SID: ' . $result['sid']);
                    } else {
                        $this->addFlash('error', '❌ Échec SMS: ' . ($result['error'] ?? 'Erreur inconnue'));
                    }
                } else {
                    $this->addFlash('error', '❌ SMS non envoyé — Ajoutez un numéro de téléphone dans votre profil.');
                }
            } else {
                $this->addFlash('success', "✅ Dépense normale — {$montant} DT ≤ {$seuil} DT");
            }
